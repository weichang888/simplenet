...
def compute_metrics(outputs, targets):
    ...
    if isinstance(targets[0], int):
        ...
        try:
            ...
            print(f"Error in flattening targets: {e}")
    ...
    # 检查是否是多分类问题
    num_classes = len(set(flat_targets))
    if num_classes > 2:
        # 确保outputs的形状正确，应该是 (num_samples, num_classes)
        if outputs.ndim == 1:  # Adding a check to prevent errors
            outputs = np.expand_dims(outputs, axis=-1)
        if outputs.shape[1] == 1:
            outputs = np.tile(outputs, (1, num_classes))
        outputs = np.exp(outputs) / np.sum(np.exp(outputs), axis=1, keepdims=True)
        print(f"Transformed outputs shape (multiclass): {outputs.shape}")

        flat_targets_bin = label_binarize(flat_targets, classes=range(num_classes))
        ...
        for i in range(num_classes):
            roc_auc = metrics.roc_auc_score(flat_targets_bin[:, i], outputs[:, i], multi_class='ovr')
            average_precision = metrics.average_precision_score(flat_targets_bin[:, i], outputs[:, i])
            f1 = metrics.f1_score(flat_targets_bin[:, i], (outputs[:, i] > 0.1).astype('int32'))
            accuracy = metrics.accuracy_score(flat_targets_bin[:, i], (outputs[:, i] > 0.1).astype('int32'))
        ...
        average_precision = np.mean(average_precision_scores)
        f1 = np.mean(f1_scores)
        accuracy = np.mean(accuracy_scores)
    else:
        print(f"Outputs shape (binary): {outputs.shape}")
        roc_auc = metrics.roc_auc_score(flat_targets, outputs)
        precision, recall, _ = metrics.precision_recall_curve(flat_targets, outputs)
        average_precision = metrics.auc(recall, precision)
        f1 = metrics.f1_score(flat_targets, (outputs > 0.1).astype('int32'))
        accuracy = metrics.accuracy_score(flat_targets, (outputs > 0.1).astype('int32'))

    return roc_auc, average_precision, f1, accuracy
    ...
    if num_classes > 2:
        ...
        for i in range(num_classes):
            roc_auc = metrics.roc_auc_score(flat_targets_bin[:, i], outputs[:, i])
            average_precision = metrics.average_precision_score(flat_targets_bin[:, i], outputs[:, i])
            f1 = metrics.f1_score(flat_targets_bin[:, i], (outputs[:, i] > 0.1).astype('int32'))
            accuracy = metrics.accuracy_score(flat_targets_bin[:, i], (outputs[:, i] > 0.1).astype('int32'))
